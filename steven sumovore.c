...
{
//enum motor_speed_setting { rev_fast, rev_medium_fast, rev_medium, rev_medium_slow, rev_slow, stop, slow, medium_slow, medium, medium_fast, fast};
    const static int motor_speeds[] = { -800, -725, -650, -550, -400, 0, 400, 550, 650, 725, 800};
    int duty_cycle;
...
{
        SeeLine.b.Left = ( adc(RLS_LeftCH0) > (threshold + 50));      // adc() prototye in sumovore.h
        SeeLine.b.CntLeft = ( adc(RLS_CntLeftCH1) > (threshold - 10) );   // adc() code in sumovore.c
        SeeLine.b.Center = ( adc(RLS_CenterCH2) > (threshold + 75) );    //  ledx turns on when corresponding 
        SeeLine.b.CntRight = ( adc(RLS_CntRightCH3) > (threshold) );  //    reflective sensore sees a line
        SeeLine.b.Right = ( adc(RLS_RightCH4) > (threshold + 200) );
}
void set_leds(void)
{
